...
class menus_n_modes(parent):
    ...
    def __init__(self):
        self.parent_object = (
            parent()
        )  # an instance of the parent class to use it's functions
        ...
        self.member_reports = (
            mp()
        )  # JO: an instance of member reports class to use its functions

        self.provider_services_logger = provider_services_logger()
    ...
    def manager_menu(self):
        ...
        while choice != 14:
            ...
            if choice == 2:
                self.member_reports.generate_member_report()  # JO
            if choice == 5:
                self.profile_edits.add_new_doctor_profile()
            if choice == 6:
                self.profile_edits.add_new_member_profile()
            if choice == 7:
                self.profile_edits.edit_doctor_profile()
            if choice == 8:
                self.profile_edits.edit_member_profile()
            if choice == 9:
                self.profile_edits.remove_doctor_profile()
            if choice == 10:
                self.profile_edits.remove_member_profile()
            if choice == 11:
                self.profile_edits.edit_member_status()
            if choice == 12:
    ...
    def provider_menu(self):
        ...
        while choice != 3:
            ...
            if choice == 1:
                self.provider_services_logger.log_member_services()

            if choice == 2:
                self.services.view_service_directory()
